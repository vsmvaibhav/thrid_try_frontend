...
const TaskForm = ({ onAddTask, editingTask, onClose, userObjectId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await fetch("https://secont-attempt.onrender.com/add-task", {
      method: "POST",
      headers: {
  ...
  return formOpen ? (
    ...
    <form className="Task_form"  onSubmit={handleSubmit}>
      <span className="close_icon" style={{color:"#000",display:"flex",justifyContent:"right"}} onClick={handleCloseForm}>
        &#10006;
      </span>
      <label className="taskForm_label">
        Task:
        <input className="taskForm_input"
          type="text"
          value={task}
      ...
      <label className="taskForm_label">
        Priority:
        <select className="taskForm_select" value={priority} onChange={(e) => setPriority(e.target.value)}>
      ...
      <button className="Addtask_button" type="submit">Add Task</button>
      </form>
