...
export default function UserHome({ userData, fetchUserData }) {
  ...
  const deleteTask = async (taskId) => {
    try {
      const response = await fetch(`https://secont-attempt.onrender.com/deleteTask/${taskId}`, {
        method: "DELETE",
        headers: {
  ...
  const handleDeleteTask = (index, taskId) => {
    ...
    fetch(`https://secont-attempt.onrender.com/deleteTask/${taskId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const filteredTasks = tasks.filter(
    (task) => (filterPriority === "all" ? true : task.priority === filterPriority)
  );
  ...
  const fetchFilteredTasks = (priority) => {
    ...
    fetch(`https://secont-attempt.onrender.com/getUserTasksWithPriority`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const fetchTasks = async () => {
    const result = await fetch(`https://secont-attempt.onrender.com/getUserTasks/${userData._id}`);
  ...
  return (
    ...
    <div className="container-fluid">
      ...
      <div className="collapse navbar-collapse" id="navbarNav">
        <div className="navbar-nav ms-auto">
          <div className="nav-item">
            <input
            style={{boxShadow: "2px 4px 6px 0.5px"}}
              type="text"
          ...
          <div className="nav-item">
            <button className="btn btn-outline-success" type="button" style={{backgroundColor: "#EC8F5E", color: "black", marginLeft:"0", border: "black",boxShadow: "2px 4px 6px 0.5px"}}>
              Search
            </button>
          </div>

          <div className="nav-item">
            <button className="btn btn-primary" style={{backgroundColor: "#EC8F5E", color: "black", border: "black", boxShadow: "2px 4px 6px 0.5px"}} onClick={() => setShowTaskForm(true)}>
              <BsPlus /> Add Task
            </button>
          </div>
          ...
          <div className="nav-item">
            <button
            style={{backgroundColor: "#EC8F5E", color: "black", border: "black", boxShadow: "2px 4px 6px 0.5px"}}
      ...
      {showTaskForm && <TaskForm onAddTask={handleAddTask} editingTask={editingTask} onClose={() => setShowTaskForm(false)} />}
